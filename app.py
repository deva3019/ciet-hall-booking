...
def user_signup():
    try:
        ...
        if data['role'] == 'administrator':
            return jsonify({'message': 'Administrator account creation not allowed here.'}), 403
        db = get_database()
        users = db['users']
...
def create_asset():
    ...
    asset_doc = {'name': name, 'seats': seats, 'description': description, 'image_url': image_url}
...
# Manual call for seeding initial assets on app start
def seed_assets():
    db = get_database()
    assets = db['assets']
    if assets.count_documents({}) == 0:
        initial_assets = [
            {"name": "Auditorium", "seats": 500, "description": "Large events, ceremonies, and cultural programs", "image_url": "/static/images/auditorium.png"},
            {"name": "Seminar Hall", "seats": 250, "description": "Workshops, talks, seminars, and presentations", "image_url": "/static/images/seminar.png"},
            {"name": "Board Room", "seats": 60, "description": "Meetings, reviews, and administrative sessions", "image_url": "/static/images/board.png"}
        ]
...
if __name__ == '__main__':
    seed_assets()  # Run this manually before starting the app to avoid decorator issue
